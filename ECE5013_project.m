...
for k=0:Np-1
    ...
    target=[R0*cosd(theta) R0*sind(theta)-k*Tp*v]; %TODO add back velocity
    ...
    Ac = sqrt(Pt*Gt*Gr*lambda^2*RCS/(4*pi)^3/Rup1^2/Rdown^2)/2;
    ...
    Ac = sqrt(Pt*Gt*Gr*lambda^2*RCS/(4*pi)^3/Rup2^2/Rdown^2)/2;
